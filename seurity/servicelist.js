...
const ServicesList = () => {
  ...
  const handleSubmit = async () => {
    if (formData.name && formData.title && formData.description && formData.category) {
      ...
      serviceData.append("technologyweuse", JSON.stringify(formData.technologyweuse));
      ...
      if (formData.images) serviceData.append("images", formData.images);
      if (formData.serviceIcon) serviceData.append("serviceIcon", formData.serviceIcon);
  
      try {
        let newServiceId;
        if (formData.serviceId) {
          const action = await dispatch(updateService({ 
          ...
          const action = await dispatch(createService(serviceData)).unwrap();
          newServiceId = action.serviceId;
          toast.success("New service added!");
        }
  
        dispatch(fetchServices());
  
        if (formData.images || formData.serviceIcon) {
  ...
  const handleFileChange = (e, type) => {
    ...
    if (file && ["image/jpeg", "image/png", "image/jpg", "image/webp"].includes(file.type)) {
      if (type === "image") {
        ...
        setFormData({ ...formData, serviceIcon: file });
      }
    } else {
